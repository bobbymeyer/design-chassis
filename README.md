...
| The gallery | `/gallery`, behind the door and not in production: the same screen of every tool side by side, and the Badger editor in its last row, built where it is built and drawn where it is not. `UI-ALIGNMENT.md` says what was aligned and where each pattern lives |
